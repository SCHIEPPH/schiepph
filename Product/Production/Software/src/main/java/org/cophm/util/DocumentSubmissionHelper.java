 ...
 * @author dunnek
 */
public class DocumentSubmissionHelper {
    ...
    public DocumentSubmissionHelper()
    {
    ...
    public RegistryResponseType createErrorResponse(RegistryErrorList errorList)
    {
        ...
        logger.debug("begin createErrorResponse()");
    ...
    public RegistryErrorList validateDocumentMetaData(ProvideAndRegisterDocumentSetRequestType body)
    {
        ...
        logger.debug("begin validateDocumentMetaData()");
        if(body == null)
        {
    ...
    public List<String> getIntendedRecepients(ProvideAndRegisterDocumentSetRequestType body)
    {
        ...
        logger.debug("begin getIntendedRecepients()");
        ...
        {
            logger.error("Unable to pull intended recipients" + ex.getMessage());
        }


        logger.debug("Found " + result.size() + " recipients");
        return result;
    }


    public RegistryResponseType buildResponseMessage(String value)
    {
        ObjectFactory factory = new ObjectFactory();
            ...
            try
            {
                ...
                //javax.xml.bind.JAXBContext jaxbContext = javax.xml.bind.JAXBContext.newInstance(response.getClass());
               javax.xml.bind.JAXBContext context = javax.xml.bind.JAXBContext.newInstance(RegistryResponseType.class);

                Unmarshaller unmarshaller = context.createUnmarshaller();


                //Unmarshaller unmarshaller = jaxbContext.createUnmarshaller();
                //JAXBElement<RegistryResponseType> respon = (RegistryResponseType)unmarshaller.unmarshal(xmlReader);
                //JAXBElement<RegistryResponseType> responseElement = unmarshaller.unmarshal(xmlReader, JAXBElement<RegistryResponseType>);


                response = (RegistryResponseType) unmarshaller.unmarshal(xmlSource);
    ...
    public String buildResponseXML(RegistryResponseType response) {
        ...
        try
        {
            ...
            JAXBElement element = factory.createRegistryResponse(response);
    ...
    public String buildProvideAndRegisterDocumentSetRequestTypeXML(ProvideAndRegisterDocumentSetRequestType body)
    {
   ...
   public String buildProvideAndRegisterDocumentSetRequestXML(AdapterProvideAndRegisterDocumentSetRequestType body) {

        String message = "";
        try
        {
            StringWriter writer = new StringWriter();
            QName qName = new QName("urn:gov:hhs:fha:nhinc:common:nhinccommonadapter","Adapter_ProvideAndRegisterDocumentSetRequest");
            JAXBElement element = new JAXBElement(qName,body.getClass(),body);
            //JAXBElement element = factory.createAdapterProvideAndRegisterDocumentSetRequest(body);
...
//    }
    ...
    protected boolean checkIdsMatch()
    {
        ...
        try
        {
            checkIds = PropertyAccessor.getPropertyBoolean("XDR.CheckPatientIdsMatch");
        }
        catch (Exception ex)
        {
            logger.error("Unable to load XDR.CheckPatientIdsMatch");
        }
    ...
    protected  String[] getSupportedMimeTypes()
    {
        ...
        try
        {
            String list = PropertyAccessor.getProperty("XDR.SupportedMimeTypes");
    ...
    public RegistryError createRegistryError(String errorCode, String severity, String codeContext)
    {
    ...
    private SlotType1 getNamedSlotItem(List<SlotType1> slots, String name)
    {
        ...
        logger.debug("begin getNamedSlotItem()");
        for(SlotType1 slot : slots)
        {
            if(slot.getName().equalsIgnoreCase(name))
            {
                result = slot;
                logger.info("Slot=" + result.getName());
                break;
            }
        }
