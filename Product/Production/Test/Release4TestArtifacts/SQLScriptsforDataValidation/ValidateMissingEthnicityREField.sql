...
# Validate data makes it to the schiepph.syndromic_surveillance table with missing Ethnicity data:
select * from schiepph.syndromic_surveillance 
where facility_identifier = '1234567890'
and facility_name = 'TestMissingEthnicity'
...
and discharge_disposition = '09';

# Validate data makes it to the schiepph.patient_Ethnicity table with missing Ethnicity data:
select * from schiepph.patient_race
where ss_id = ( select ss_id from schiepph.syndromic_surveillance where facility_name='TestMissingEthnicity' )
and concept_code = '2106-3'
and concept_name = 'White'
and coding_system = 'CDCREC';

# Validate data makes it to the schiepph.injury_codes table with missing Ethnicity data:
...
# Validate data makes it to the schiepph.chief_complaint table with missing Ethnicity data:
...
# Validate data makes it to the schiepph.unique_patient_identifier table with missing Ethnicity data:
select * from schiepph.unique_patient_identifier
where ss_id = ( select ss_id from schiepph.syndromic_surveillance where facility_name='TestMissingEthnicity' )
and identifier_type_code = 'PI'
and identifier = '95101100001';

# Validate NO data makes it to the schiepph.patient_ethnicity table with missing Ethnicity data:
select count(*) from schiepph.patient_ethnicity 
where ss_id = ( select ss_id from schiepph.syndromic_surveillance where facility_name='TestMissingEthnicity' );
