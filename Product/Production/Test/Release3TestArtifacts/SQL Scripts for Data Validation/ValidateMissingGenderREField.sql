...
# Validate data makes it to the schiepph.syndromic_surveillance table with missing gender data:
select * from schiepph.syndromic_surveillance 
where facility_identifier = '1234567890'
and facility_name = 'TestMissingGender'
...
# Validate data makes it to the schiepph.patient_race table with missing gender data:
select * from schiepph.patient_race 
where ss_id = ( select ss_id from schiepph.syndromic_surveillance where facility_name='TestMissingGender' )
and concept_code = '2106-3'
and concept_name = 'White'
and coding_system = 'CDCREC';

# Validate data makes it to the schiepph.patient_ethnicity table with missing gender data:
...
# Validate data makes it to the schiepph.injury_codes table with missing gender data:
...
# Validate data makes it to the schiepph.chief_complaint table with missing gender data:
...
# Validate data makes it to the schiepph.unique_patient_identifier table with missing gender data:
