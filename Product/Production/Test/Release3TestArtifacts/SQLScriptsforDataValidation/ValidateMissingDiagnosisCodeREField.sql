...
# Validate data makes it to the schiepph.syndromic_surveillance table with missing Diagnosis Code data:
select * from schiepph.syndromic_surveillance 
where facility_identifier = '1234567890'
and facility_name = 'TestMissingDiagnosisCode'
...
# Validate data makes it to the schiepph.patient_Ethnicity table with missing Diagnosis Code data:
select * from schiepph.patient_race
where ss_id = ( select ss_id from schiepph.syndromic_surveillance where facility_name='TestMissingDiagnosisCode' )
...
# Validate data makes it to the schiepph.injury_codes table with missing Diagnosis Code data:
...
# Validate data makes it to the schiepph.chief_complaint table with missing Diagnosis Code data:
...
# Validate data makes it to the schiepph.unique_patient_identifier table with missing Diagnosis Code data:
